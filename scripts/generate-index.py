#!/usr/bin/env python3
"""
Generate TEMPLATE_REGISTRY.json from the template tree, plus the two
human-facing views that quote its numbers: templates/TEMPLATE_REGISTRY.md
and the template/language count badges in README.md.

Everything a reader sees is derived from the same scan in the same run, so
the human index and the badges cannot drift from the registry the way they
did while the index was maintained by hand.
...
# Human-facing section titles for the categories in templates/. A category
# missing here still renders, under its directory name title-cased, so adding
# a category never silently drops it from the index.
CATEGORY_TITLES = {
    "ai": "AI / LLM Security",
...
GENERATED_BANNER = ("<!-- generated by scripts/generate-index.py - "
                    "do not edit by hand -->")

# The badges in README.md whose numbers this script owns.
BADGE_RES = {
    "templates": re.compile(r"(img\.shields\.io/badge/templates-)\d+"),
    "languages": re.compile(r"(img\.shields\.io/badge/languages-)\d+"),
}
...
def heading_anchor(heading):
    """GitHub's own slug for a heading: lowercase, drop punctuation, space
    becomes hyphen. Computed from the rendered heading so the contents table
    and the sections it points at can never disagree."""
    slug = "".join(ch for ch in heading.lower()
...
def skeleton_rows(repo_root):
    """(language, skeleton, ai notes) for templates/skeleton/, or []."""
    skel = repo_root / "templates" / "skeleton"
    if not skel.is_dir():
        return []
    rows = {}
    for path in sorted(skel.iterdir()):
        if not path.is_file():
            continue
        for marker, slot in (("-template-skeleton", 0), ("-template-ai-notes", 1)):
            if marker in path.name:
                lang = path.name.split(marker, 1)[0]
                entry = rows.setdefault(lang, ["", ""])
                entry[slot] = "templates/skeleton/%s" % path.name
    return [(lang, entry[0], entry[1]) for lang, entry in sorted(rows.items())]


def render_registry_markdown(repo_root, registry):
    """The human index: one table per category, straight from the scan."""
    ...
    lines = [
        ...
        "",
        "> **Total templates:** %d | **Languages:** %d | **Categories:** %d"
        % (registry["total_templates"], len(registry["languages"]),
    ...
    headings = {
        category: "%d. %s (`%s/`)" % (number, category_title(category), category)
        for number, (category, _count) in enumerate(categories, 1)
    }
    for category, count in categories:
        lines.append("| %s | [%s](#%s) | %d |" % (
            headings[category].split(".", 1)[0],
            md_escape(category_title(category)),
            heading_anchor(headings[category]), count,
        ))
    lines.append("")
    lines.append("---")
    ...
    for category, _count in categories:
        ...
        for row in by_category.get(category, []):
            flags = "" if row["runnable"] else " **(not runnable)**"
            lines.append("| `%s` | %s%s | %s | %s | `%s` |" % (
                md_escape(row["id"]), md_escape(row["name"]), flags,
                row["severity"], row["language"], md_escape(row["path"]),
            ))
        lines.append("")
        lines.append("---")

    skeletons = skeleton_rows(repo_root)
    if skeletons:
        lines += [
            "",
        ...
        for lang, skeleton, notes in skeletons:
            lines.append("| %s | %s | %s |" % (
                lang,
                "`%s`" % skeleton if skeleton else "-",
    ...
    for language, count in registry["languages"].items():
        lines.append("| %s | %d |" % (language, count))
    lines.append("")
    return "\n".join(lines)


def write_registry_markdown(repo_root, registry):
    out = repo_root / "templates" / "TEMPLATE_REGISTRY.md"
    out.write_text(render_registry_markdown(repo_root, registry),
                   encoding="utf-8")
    return out


def update_readme_badges(repo_root, registry):
    """Rewrite only the digits in the templates-/languages- shields URLs."""
    out = repo_root / "README.md"
    if not out.is_file():
        return None, {}
    text = original = out.read_text(encoding="utf-8")
    values = {
        "templates": registry["total_templates"],
        "languages": len(registry["languages"]),
    }
    updated = {}
    for badge, value in values.items():
        text, n = BADGE_RES[badge].subn(r"\g<1>%d" % value, text)
        updated[badge] = (n, value)
    if text != original:
...
def main():
    ...
    md = write_registry_markdown(repo_root, registry)
    print("Wrote %s" % md.relative_to(repo_root))

    readme, badges = update_readme_badges(repo_root, registry)
    if readme is not None:
        for badge, (hits, value) in sorted(badges.items()):
            if hits:
                print("  README badge %-9s -> %d" % (badge, value))
            else:
                print("  WARNING: README badge '%s' not found - the shields URL "
                      "may have been renamed" % badge, file=sys.stderr)
